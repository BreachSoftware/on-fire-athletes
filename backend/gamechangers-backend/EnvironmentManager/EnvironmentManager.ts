 ...
 */
const AWS_DEVELOPMENT_API_PREFIX = "uxt0zaw7b0";
const AWS_PRODUCTION_API_PREFIX = "5lii8dnt7k";
...
class EnvironmentManager {
	...
	private constructor() {
		...
		this.apiConfigs = {
			...
			[Environment.Local]: {
				baseUrl: AWS_DEVELOPMENT_OVERRIDE_PREFIX === undefined ?
					"http://localhost:4000/" :
		...
		this.dbConfigs = {
			[Environment.Development]: {
				tableNames: DB_TABLES.reduce((acc, table) => {
 ...
 */
export const apiEndpoints = API_ENDPOINTS.reduce((acc, endpoint) => {
	acc[endpoint] = () => {
		return environmentManager.getApiEndpoint(endpoint).replace(/_/g, "/");
	};
	return acc;
 ...
 */
export const dbTables = DB_TABLES.reduce((acc, table) => {
	acc[table] = () => {
