 ...
 */
export const getUUID: Handler = async (
	...
	try {
		...
		if (!email) {
			...
			return sendResponse(400, {
				error: "The request requires a email body parameter",
			});
		}
		...
		const input = {
			UserPoolId: "us-east-1_Awb6vGmOg", // required
		...
		const protectedString = response.UserAttributes[0].Value
			? response.UserAttributes[0].Value
			: "No user found";
		...
		console.error(
			"The following error occurred when retrieving a profile: ",
		);
		console.error(error);

		// Return an error response with a 500 status code
		return sendResponse(500, {
			error: "An error occurred when retrieving a profile",
		});
	}
};
