 ...
 */
const getObjectUrlsByPrefix = async (
  bucketName: string,
  prefix: string
): Promise<string[]> => {
  const s3 = new S3()
...
export const getUserMedia: Handler = async (
  ...
  if (!userId || !authToken || !mediaType) {
    return Promise.resolve({
      ...
      body: JSON.stringify({
        success: false,
        message:
          "Error! Missing one or more required query parameters. authtoken, userId, or mediaType is null.",
      }),
    })
  } else if (
    ...
    return Promise.resolve({
      ...
      body: JSON.stringify({
        success: false,
        message:
          "Error! Incorrect type for one or more query parameters. authToken, userId, and mediaType should be strings.",
      }),
    })
  }
  ...
  if (mediaType !== "image" && mediaType !== "video") {
    return Promise.resolve({
      ...
      body: JSON.stringify({
        success: false,
        message:
          "Error! Invalid media type. Media type must be either 'image' or 'video'.",
      }),
    })
  }
  ...
  try {
    const bucketName = "onfireathletes-media-uploads"
