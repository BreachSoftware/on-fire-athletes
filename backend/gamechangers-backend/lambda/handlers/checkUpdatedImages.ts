...
export const checkUpdatedImages: Handler = async (
  ...
  if (!userId || !authToken || !lastChecked) {
    return Promise.resolve({
      ...
      body: JSON.stringify({
        success: false,
        message:
          "Error! Missing one or more required query parameters. authToken, userId, or lastChecked is null.",
      }),
    })
  } else if (
    ...
    return Promise.resolve({
      ...
      body: JSON.stringify({
        success: false,
        message:
          "Error! Incorrect type for one or more query parameters. authToken, userId, and lastChecked should be strings.",
      }),
    })
  }
  ...
  try {
    ...
    const objectUrls = await getObjectUrlsByPrefix(
      bucketName,
      prefix,
      lastCheckedDate
    )
