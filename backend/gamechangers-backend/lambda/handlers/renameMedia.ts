...
export const renameMedia: Handler = async (
  ...
  if (
    ...
    data.cardBackImage === undefined
  ) {
    return {
      statusCode: 400,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        error: "One or more required attributes is undefined",
      }),
    }
  } else if (
    ...
    typeof data.cardBackImage !== "string"
  ) {
    return {
      statusCode: 400,
      headers: { "Content-Type": "application/json" },
  ...
  const cardBackImageSuccess = await renameS3Object(
    bucketName,
    cardBackImage,
    userId
  )
