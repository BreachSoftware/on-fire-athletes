...
export const assignAvatar: Handler = async (
  ...
  if (!userId || !authToken) {
    return Promise.resolve({
      ...
      body: JSON.stringify({
        success: false,
        message:
          "Error! Missing one or more required query parameters. authtoken or userId is null.",
      }),
    })
    ...
    return Promise.resolve({
      ...
      body: JSON.stringify({
        success: false,
        message:
          "Error! Incorrect type for one or more query parameters. authToken and userId should be strings.",
      }),
    })
  }
