...
export const createSetupIntent: Handler = async (
	...
	if (data.customerId === undefined) {
		return sendResponse(400, {
			error: "The request body must contain the customerId property",
		});
	} else if (data.customerId === "") {
		return sendResponse(400, {
			error: "The customerId property cannot be empty",
		});
	} else if (typeof data.customerId !== "string") {
		return sendResponse(400, {
			error: "The customerId property must be of type string",
		});
	}
