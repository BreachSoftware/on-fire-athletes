...
import { getWithExpiry, setWithExpiry } from "@/components/localStorageFunctions";
...
export function allPartsToRecolor(): number[] {
	return Object.keys(CardPart).map(Number).filter((value) => {
...
export default class TradingCardInfo {
	...
	tradeStatus?: TradeStatus
		...
		this.backgroundAccentColor = options?.backgroundAccentColor || "#2a2a2a";
	 ...
	 */
	static async submitCard(cardInfo: TradingCardInfo, user_id: string, renameMedia: boolean = true) : Promise<TradingCardInfo> {
		const myHeaders = new Headers();
		myHeaders.append("Content-Type", "application/json");

		// Check if the front photo S3 URL is empty, if it is, use the front photo URL
		if (cardInfo.frontPhotoS3URL === undefined || cardInfo.frontPhotoS3URL === "") {
			if (cardInfo.frontPhotoURL !== undefined && cardInfo.frontPhotoURL !== "") {
				cardInfo.frontPhotoS3URL = cardInfo.frontPhotoURL;
			}
		}

		// Check if the back video S3 URL is empty, if it is, use the back video URL
		if (cardInfo.backVideoS3URL === undefined || cardInfo.backVideoS3URL === "") {
			if (cardInfo.backVideoURL !== undefined && cardInfo.backVideoURL !== "") {
				cardInfo.backVideoS3URL = cardInfo.backVideoURL;
			} else {
				cardInfo.backVideoS3URL = "https://gamechangers-media-uploads.s3.amazonaws.com/";
			}
		}

		// Check if the card back image S3 URL is empty, if it is, use the card back image URL
		if (cardInfo.cardBackS3URL === undefined || cardInfo.cardBackS3URL === "") {
			if (cardInfo.cardBackS3URL !== undefined && cardInfo.cardBackS3URL !== "") {
		...
		if (renameMedia) {
			...
			const frontPhotoKey = cardInfo.frontPhotoS3URL.split("https://gamechangers-media-uploads.s3.amazonaws.com/")[1];
			const backVideoKey = cardInfo.backVideoS3URL.split("https://gamechangers-media-uploads.s3.amazonaws.com/")[1];
			const cardBackImageKey = cardInfo.cardBackS3URL.split("https://gamechangers-media-uploads.s3.amazonaws.com/")[1];

			const newURLs = await TradingCardInfo.renameMedia(user_id,
				frontPhotoKey,
				backVideoKey,
				cardBackImageKey
			);
			...
			if (newURLs.cardBackImage !== undefined && newURLs.cardBackImage !== "") {
		...
		const savedCard = await (await fetch(apiEndpoints.createCard(), requestOptions)).json();
	...
	static async renameMedia(
		user_id: string, oldCardImageKey: string, oldVideoKey: string, oldCardBackImageKey: string
		...
		try {
			...
			await fetch(apiEndpoints.renameMedia(), renameURLRequestOptions)
				...
				.then((result) => {
					returnVal.cardImage = result.cardImage !== "" ? result.cardImage : "";
					returnVal.video = result.video !== "" ? result.video : "";
					returnVal.cardBackImage = result.cardBackImage !== "" ? result.cardBackImage : "";
