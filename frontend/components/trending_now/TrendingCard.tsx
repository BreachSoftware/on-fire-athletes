import { Box, Flex, Skeleton, IconButton, Text, VStack, Divider, Button, HStack } from "@chakra-ui/react";
...
import { hexToRgb, hslToRgb, rgbToHex, rgbToHsl, saturateHex } from "../create/OnFireCard/card_utils";
...
interface TrendingCardProps {
	...
	overrideCardClick?: (() => void) | ((card: TradingCardInfo | SerializedTradingCard) => void);
...
export default function TrendingCard({
	...
	if (typeof window !== "undefined") {
		...
		windowAdjustedDefaultWidth = Math.min(windowAdjustedDefaultWidth, window.innerWidth * 0.9);
	}
	...
	const card = passedInCard instanceof SerializedTradingCard ? passedInCard.TradingCardInfo : passedInCard;

	const [ buttonText, setButtonText ] = useState<string | null>(null);
	const [ buttonAction, setButtonAction ] = useState<(() => void) | null>(null);
	const [ disableActionButton, setDisableActionButton ] = useState<boolean>(false);
	...
	useEffect(() => {
		if (onProfilePage) {
			...
			if (tabName === TabName.Created && card.currentlyAvailable <= 0) {
				...
				if (card.price !== 0 && card.currentlyAvailable > 0 && currentUser !== card.generatedBy) {
			...
			setButtonText(
				card.currentlyAvailable <= 0 ? "Card Unavailable" : currentUser === card.generatedBy ? "Send Card" : card.price ? "Buy Now" : "Request Trade"
			);
			setDisableActionButton(false);
		}
		...
		return (
			<Box
				...
				onClick={() => {
					if (onCardClick) {
						...
						router.push(`/profile?user=${card?.generatedBy}&card=${card?.uuid}`);
					}
				}}
				display="flex"
			...
			>
				...
				>
					...
					<Skeleton isLoaded={!isPageFlipping} fadeDuration={1} borderRadius={"12"}>
						<OnFireCard key={card.uuid} slim card={card} showButton={false} ref={onFireCardRef} shouldFlipOnClick />
					</Skeleton>
				</Box>
			</Box>
		);
	...
	return (
		<>
			<VStack width={{ base: "100%", md: trendingCardWidth ?? "auto" }}>
				{onProfilePage ? (
					<>
						<Link activeClass="active" to="cardElement" spy smooth offset={-50} duration={500} width="inherit">
				...
				{!slim && (
					<Flex
						...
						w={{ base: "100%", md: DYNAMIC_SIZING ? trendingCardWidth ?? windowAdjustedDefaultWidth : windowAdjustedDefaultWidth }}
						maxW={windowAdjustedDefaultWidth}>
						{/* VStack component for the trading card details */}
						<VStack align={"left"} color={"white"} gap={4} fontSize={14} width={"100%"} letterSpacing={"0px"} paddingTop={2} overflowX="hidden">
							<HStack justifyContent={"space-between"}>
								<Text fontSize={18} fontFamily={"Barlow Semi Condensed"} fontWeight={"bold"}>
									{card.price ? `$${card.price.toFixed(2)}` : "TRADE ONLY"}
								</Text>
								{shouldShowButton && (
									<IconButton
										...
										background={"white"}
										marginLeft={5}
										icon={<FontAwesomeIcon icon={faArrowsRotate} color="black" style={{ width: "15px", height: "15px" }} />}
									/>
								)}
							</HStack>
							...
							<VStack align={"left"} gap={0} lineHeight={"20px"}>
								...
								<Text textTransform={"uppercase"} fontSize="20px" fontFamily="Barlow Semi Condensed" fontWeight="extrabold">
									{`${card.firstName} ${card.lastName}`}
								</Text>

								{/* Text component for the player's sport, position, and team */}
								<Text fontSize="17px" fontFamily="Helvetica Neue" fontWeight={400}>
								...
								{passedInCard instanceof SerializedTradingCard ? (
									<>
										...
										<Text color={"gray"}>{`Card Number: ${passedInCard.serialNumber} of ${card.totalCreated}`}</Text>
									</>
								) : (
									<Text color={"gray"} fontSize="17px">{`${card.currentlyAvailable} of ${card.totalCreated} available`}</Text>
								)}
							</VStack>
							{buttonText && (
								<Button
