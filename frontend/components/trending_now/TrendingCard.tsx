...
import { Flex, IconButton, Text, VStack, Divider, Button, HStack, } from "@chakra-ui/react";
...
export default function TrendingCard({
	...
	useEffect(() => {
		if (onProfilePage) {
			...
			setButtonText(card.currentlyAvailable <= 0 ? "Card Unavailable" :
				currentUser === card.generatedBy ? "Send Card" :
					card.price ? "Buy Now" : "Request Trade"
			);
			setDisableActionButton(false);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [ currentUser, card, tabName, setCurrentCard, onSendCardModalOpen, loginModalOpen,
		onAddToCollectionModalOpen, currentUserId, onProfilePage, privateView ]);
				...
				<Flex align={"center"} justifyContent={"space-between"} w="100%">
					...
					<VStack align={"left"} color={"white"} gap={4} fontSize={14} width={"100%"} letterSpacing={"0px"} paddingTop={2} overflowX="hidden">
						...
						<VStack align={"left"} gap={0} lineHeight={"20px"}>
							...
							<Text textTransform={"uppercase"} fontSize={18} fontFamily={"Helvetica"} fontWeight={"bolder"}>
								{`${card.firstName} ${card.lastName}`}
							</Text>

							{/* Text component for the player's sport, position, and team */}
							<Text noOfLines={1}>{card.position} / {card.teamName}</Text>
							...
							{passedInCard instanceof SerializedTradingCard ? (
								...
								<Text color={"gray"}>{`${card.currentlyAvailable} of ${card.totalCreated} available`}</Text>
							}
						</VStack>
						{
							buttonText && <Button
