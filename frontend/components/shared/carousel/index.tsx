 ...
 */
export default function SharedCarousel({ containerOverrides, isLightMode, hideDots, children }: Props) {
		...
		<Box {...containerOverrides}>
			<Carousel
				...
				renderArrowPrev={(onClick, hasPrev) => {
					return <CarouselArrow icon={<CarouselLeftIcon />} isLightMode={isLightMode} isPrev onClick={onClick} isDisabled={!hasPrev} />;
				}}
				renderArrowNext={(onClick, hasNext) => {
					return <CarouselArrow icon={<CarouselRightIcon />} isLightMode={isLightMode} onClick={onClick} isDisabled={!hasNext} />;
				}}>
				{children}
			</Carousel>
			{!hideDots && <CarouselDots selectedIndex={selectedIndex} isLightMode={isLightMode} itemsLength={children.length} />}
		</Box>
	);
}
