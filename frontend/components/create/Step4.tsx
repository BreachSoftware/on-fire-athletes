import { VStack, Text, HStack, Image, Button, Flex, Grid, GridItem, Spacer, Tooltip, Box } from "@chakra-ui/react";
...
export default function Step4() {
	...
	const [ backgroundMainColor, setBackgroundMainColor ] = useState(card.curCard.backgroundMainColor);
	const [ backgroundAccentColor, setBackgroundAccentColor ] = useState(card.curCard.backgroundAccentColor);
	const [ backgroundTextColor, setBackgroundTextColor ] = useState(card.curCard.backgroundTextColor);
	const [ selectedBackground, setSelectedBackground ] = useState(card.curCard.selectedBackground);
	...
	const [ firstNameSolid, setFirstNameSolid ] = useState(card.curCard.firstNameSolid);
	const [ lastNameSolid, setLastNameSolid ] = useState(card.curCard.lastNameSolid);

	const [ cardNameFont, setCardNameFont ] = useState(card.curCard.nameFont);

	const [ signatureColor, setSignatureColor ] = useState(card.curCard.signatureColor);

	const [ numberColor, setNumberColor ] = useState(card.curCard.numberColor);

	const numberOfBackgrounds = 20;
	...
	function backgroundSelector(index: number) {
		return (
			<Button
				...
				border={selectedBackground === index ? "2px solid var(--chakra-colors-green-100)" : ""}
				borderRadius={"5px"}
				p="2px"
				onClick={() => {
			...
			>
				<Box position="relative" height="100%" width="100%">
					<Image
						src={`/card_backgrounds/bkg_${index}.png`}
						bg={"#888"}
						objectFit={"cover"}
						height={"100%"}
						width={"100%"}
						...
						borderRadius={"5px"}
						style={{ filter: `brightness(${selectedBackground === index ? "100%" : "70%"})` }}
						draggable={false}
					/>
	...
	return (
		<>
			...
			>
				<Flex flexDirection={{ base: "column", sm:"column", md: "column", lg:"row" }} width={"100%"}>
					<VStack marginBottom={"20px"} height={"100%"} alignItems={"left"}>
						<Text
							opacity={".62"}
							width={"90%"}
						...
						<HStack justifyContent={"space-between"}>
							...
							>
								...
								>
									...
									>
										{card.curCard.number ?
											...
											>
												<Flex
													style={{ filter: "opacity(50%) grayscale(100%) brightness(0.4)", cursor: "not-allowed" }}
													alignItems={"center"}
												>
													<FullColorPicker
														type="numberColor"
								...
								<GridItem rowStart={{ base: 5, sm: 5, md: 5, lg: 5, xl: 3 }}>
									<Flex
										w={"100%"}
										alignItems={"center"}
										justifyContent={"flex-start"}
								...
								<GridItem
									...
									rowStart={{ base: 7, sm: 7, md: 7, lg: 5, xl: 3 }}
								>
									<Flex
										w={"100%"}
										alignItems={"center"}
					...
					<Spacer/>
					<VStack width={"90%"} height={"100%"} alignItems={"left"} gap={"28px"}>
						<Text
							opacity={".62"}
							width={"100%"}
							backgroundColor={"#303C3A"}
						...
						<HStack
							...
							<Text
								minW={"30%"}
								fontSize={"sm"} color={"white"} fontWeight={"bold"} transform={"skewX(-6deg)"}>Font Style:</Text>
							{/* solid or outline for card name */}
							<RadioPicker
								option1text={"Classic"}
								option2text={"Marker"}
								value={cardNameFont === "Uniser-Bold" ? "1" : "2"}
								onChange={(value) => {
									setCardNameFont(value === "1" ? "Uniser-Bold" : "'Brotherhood', sans-serif");
								}}
							/>
						</HStack>
						<HStack gap={2} >
							<Text minW={"30%"} fontSize={"sm"} color={"white"} fontWeight={"bold"} transform={"skewX(-6deg)"}>First Name:</Text>
						...
						<HStack gap={2} >
							<Text minW={"30%"} fontSize={"sm"} color={"white"} fontWeight={"bold"} transform={"skewX(-6deg)"}>Last Name:</Text>
				...
				<VStack alignItems={"left"}>
					<Text opacity={".62"}
						...
						fontWeight={"bold"}
						letterSpacing={"1.6px"}
						paddingLeft={"2%"}>Background Pattern:</Text>

					{/* 6 vertical photo placeholders */}
					<HStack mb={3} pb={5} spacing={4} width={"100%"} overflowX="scroll"
						css={{
				...
				<VStack mt={{ base: 0, xl: -6 }} alignItems={"left"} pt="10px">
					<Text opacity={".62"}
						...
						fontWeight={"bold"}
						letterSpacing={"1.6px"}
						paddingLeft={"2%"}>Background Color:</Text>
					...
					<HStack justifyContent={"space-between"} width={"100%"} flexWrap={"wrap"}>
						<Flex
							...
							flexDir={{ base: "row-reverse", sm: "row-reverse", md: "row-reverse", lg: "row" }}
						>
							<FullColorPicker type="backgroundMainColor" setColor={setBackgroundMainColor} />
							<Text>Primary</Text>
							<Spacer />
							<FullColorPicker type="backgroundAccentColor" setColor={setBackgroundAccentColor} />
							<Text>Secondary</Text>
							<Spacer />
							{card.curCard.cardType === "a" ?
								<>
									<FullColorPicker type="backgroundTextColor" setColor={setBackgroundTextColor} isDisabled={false} />
									<Text>Last Name</Text>
								</> :
								<Tooltip
								...
								>
									<Flex
										...
										alignItems={"center"}
									>
										<FullColorPicker type="backgroundTextColor" setColor={setBackgroundTextColor} isDisabled={true} />
										<Text>Last Name</Text>
									</Flex>
								</Tooltip>
							}
