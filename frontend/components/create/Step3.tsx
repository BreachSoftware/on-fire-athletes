import { HStack, Divider, Text, VStack, Spacer, Spinner } from "@chakra-ui/react";
 ...
 *
 */
export async function uploadAssetToS3(filename: string, asset: File | Blob, mediatype: string, filetype: string) {
	...
	const presignedURL = await fetch(apiEndpoints.media_generatePresignedURL(), requestOptions);
...
export default function Step3() {
	...
	const { startProcessing, stopProcessing, setMediaType } = useMediaProcessing();
		...
		try {
			...
			reader.onloadend = async() => {
				...
				card.setCurCard({
					...
					frontPhotoS3URL: `https://gamechangers-media-uploads.s3.amazonaws.com/image/${ filename }`,
	...
	async function processVideoSelect(files: FileList) {
		...
		card.setCurCard({
			...
			backVideoS3URL: `https://gamechangers-media-uploads.s3.amazonaws.com/video/${ filename }`,
		...
		return (
			<HStack>
				<HStack alignItems={"top"}>
					<Text fontSize={"14px"} fontStyle={"italic"} fontWeight={"bold"}>
						{stepNum}.
					</Text>
					<VStack alignItems={"left"} spacing={"0px"}>
						...
						<Text color={"grey"} fontSize={"10px"} fontStyle={"italic"} fontWeight={"bold"}>
							{description}
						</Text>
						<Text color={"grey"} fontSize={"10px"} fontStyle={"italic"} fontWeight={"bold"}>
				...
				{!isLoading && fileName !== "" && <CheckIcon paddingRight={1} boxSize={5} color="green.100" />}
				<DropzoneButton
	 ...
	 * @param param0
	 */
	function SignatureRow({ stepNum, title, signature }: { stepNum: string, title: string, signature: string }) {
		return (
			<HStack>
				<HStack alignItems={"top"}>
					<Text fontSize={"14px"} fontStyle={"italic"} fontWeight={"bold"}>{stepNum}.</Text>
					<VStack alignItems={"left"} spacing={"0px"}>
						<Text fontFamily="'Barlow', sans-serif" fontSize={"14px"} fontStyle={"italic"} fontWeight={"bold"}>{title}:</Text>
					</VStack>
				</HStack>
				<Spacer />
				{shouldShow(signature) && <CheckIcon paddingRight={1} boxSize={5} color="green.100" />}
				<SignatureModal buttonText="Sign now" width={"40%"} auth={auth} />
			</HStack>
		);
	}
