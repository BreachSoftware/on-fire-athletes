...
import { Box, Icon, IconButton, Slider, SliderFilledTrack, SliderThumb, SliderTrack, VStack } from "@chakra-ui/react";
...
function MySlider(sliderProps: SliderProps) {
	const startVal = sliderProps.startingValue !== undefined ? sliderProps.startingValue : 50; // Set default value to 50 if startingValue is not provided
	...
	return (
		<>
			...
			>
				<SliderTrack bg="gray" w={`${sliderWidth * 1.2}px`} clipPath={"polygon(10% 0, 90% 0, 50% 100%)"}>
...
function HeroSlider({ startingValue }: {startingValue: number}) {
	...
	return <>
		...
		<Icon mb={"30px"} h={sliderWidth} w={sliderWidth} color={"#27CE00"} boxSize={"1.6em"} as={FaImage} />
	</>;
}
...
function SignatureSlider({ startingValue }: {startingValue: number}) {
	...
	return <>
		<MySlider
			...
			maxValue={350}
			startingValue={startingValue} />
		<Box mb={"30px"} h={sliderWidth} w={sliderWidth} transform={"scale(1.3)"} >
			<FontAwesomeIcon color={"#27CE00"} icon={faSignature} />
		</Box>
	</>;
}
...
function VideoSlider({ startingValue }: {startingValue: number}) {
	...
	return <>
		<MySlider
			...
			maxValue={1500}
			startingValue={startingValue} />
		<Icon mb={"30px"} h={sliderWidth} w={sliderWidth} color={"#27CE00"} boxSize={"1.6em"} as={FaVideo} />
	</>;
}
	...
	return <>
		<IconButton
			...
			onClick={() => {
				cardHook.setCurCard({
					...cardHook.curCard,
					backVideoRotation: cardHook.curCard.backVideoRotation + 90
				});
			}}
		/>
	</>;
...
export default function OnFireCardSliders() {
	...
	const [ shouldShowPhotoSlider, setShouldShowPhotoSlider ] = useState(cardHook.curCard.frontPhotoURL !== "");
	const [ shouldShowSignatureSlider, setShouldShowSignatureSlider ] = useState(cardHook.curCard.signature !== "" && cardHook.curCard.signature !== undefined);
	const [ shouldShowVideoSlider, setShouldShowVideoSlider ] = useState(cardHook.curCard.backVideoURL !== "");
	...
	const videoSlider = <VideoSlider startingValue={cardHook.curCard.backVideoWidth} />;
	...
	if((shouldShowPhotoSlider || shouldShowVideoSlider) && !shouldShowSignatureSlider && !hasSetFirst) {

		mediaWasFirst = true;
		setHasSetFirst(true);
	} else if (!(shouldShowPhotoSlider || shouldShowVideoSlider) && shouldShowSignatureSlider && !hasSetFirst) {
	...
	useEffect(() => {
		if (cardHook.curCard.stepNumber !== 3) {
			...
			setShouldShowSignatureSlider(cardHook.curCard.signature !== "" && cardHook.curCard.signature !== undefined);
	...
	function determineMediaSlider() {
		const thingToShow = cardHook.curCard.frontIsShowing ? heroSlider : videoSlider;
	...
	return (
		<VStack>
			...
			{shouldShowSignatureSlider && hasSetFirst && cardHook.curCard.frontIsShowing && <SignatureSlider startingValue={startingSigWidth.current} />}
			{ hasSetFirst && !mediaWasFirst && determineMediaSlider()}
			{ shouldShowVideoSlider && !cardHook.curCard.frontIsShowing && rotateVideoButton }
		</VStack>
	);
}
