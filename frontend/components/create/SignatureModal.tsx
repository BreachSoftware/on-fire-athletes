...
function SignatureModal(props: SignatureModalProps, { ...rest }) {
	...
	function ModalContents() {
		...
		return (
			<ModalContent backgroundColor={"grey.700"} maxW={{ base: "600px", md: "700px" }} h={"auto"} color={"white"}>
				{width < screenThreshold ?
					<>
						<ModalBody>Screen too narrow! Rotate your device to sign your card.</ModalBody>
						<ModalCloseButton />
					</> :
					<>
						<ModalHeader alignSelf="center">Add a Signature</ModalHeader>
						<ModalBody >
							<VStack alignContent={"center"}>
								<Text>Use the box below to sign your card!</Text>
								<ReactSketchCanvas
									ref={contentsRef}
									style={canvasStyle}
									width={toPx(width - 50)}
									...
									strokeWidth={8}
									strokeColor={currentInfo.curCard.signatureColor ? currentInfo.curCard.signatureColor : "white"}
				...
				<ModalFooter>
					...
					Clear Drawing
							...
							<Button variant="ghost" onClick={() => {
								// Export paths
								contentsRef.current?.exportPaths().then((canvasPath) => {
									localStorage.setItem("prevdrawing", JSON.stringify(canvasPath));
								}).catch((error) => {
									console.error("Error exporting paths:", error);
								});
								// Export PNG. Broken when backgroundImage is set
								contentsRef.current?.exportImage("png")
									.then((URL:string) => {
										recolor(URL, currentInfo.curCard.signatureColor ? currentInfo.curCard.signatureColor : "white", headers, true)
											.then(async(newURL) => {
												const filename = await uploadSignatureToS3(newURL);
												currentInfo.setCurCard({
													...currentInfo.curCard,
													signature: newURL,
													frontIsShowing: true,
													signatureS3URL: `https://gamechangers-media-uploads.s3.amazonaws.com/signature/${ filename }`,
													partsToRecolor: [ ...currentInfo.curCard.partsToRecolor, CardPart.SIGNATURE ],
												});
											});
									})
									.catch((e:Error) => {
			...
			>
				...
				{props.inlineImage ?
					currentInfo.curCard.signature ?
						...
						<FontAwesomeIcon color={"#27CE00"} icon={faSignature} style={{ marginLeft: "10px" }} /> :
