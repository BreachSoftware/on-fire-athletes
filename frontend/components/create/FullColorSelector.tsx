import { Box, Circle, Popover, PopoverArrow, PopoverContent, PopoverTrigger, HStack, Spacer } from "@chakra-ui/react";
 ...
 */
export default function FullColorPicker({ type, setColor, isDisabled = false }: FullColorPickerProps) {
	...
	function handleColorFinalize(newColor: string): void {
		if (newColor) {
			...
			if (setColor) {
				...
				switch (type) {
				case "borderColor":
					partToRecolor.push(CardPart.EXTERIOR_BORDER, CardPart.INTERIOR_BORDER);
					break;
				case "signatureColor":
	...
	function showColor(color: string, uniqueColorsSet: Set<string>): boolean {
		if (uniqueColorsSet.has(color) || presetColorsRow1.includes(color) || presetColorsRow2.includes(color) || color == pickerColor) {
			return false;
		}
	...
	return (
		...
		<>
			{isMobile ?
				...
				<HStack>
					<input
						style={{ width: "40px", height: "40px", backgroundColor: pickerColor, border: "2px solid white" }}
				...
				<Popover onClose={() => {
					...
					{isDisabled ? null : <PopoverContent width={"100%"}>
						<PopoverArrow padding={0} margin={0} />
						<Box backgroundColor={"gray.600"} width={"100%"} display="flex" justifyContent="center">
							<HexColorPicker color={color} onChange={setPickerColor} />
						</Box>
						<Box>
							<HStack backgroundColor={"gray.600"} width={"100%"} minH={"40px"}>
								...
								{colorAttributes.map((color, index) => {
									const currentColor = String(card.curCard[color]);

									if(showColor(currentColor, uniqueColorsSet)) {
										return (
											<Fragment key={index}>
												<Circle
													cursor={"pointer"}
