...
import { mobileCardCreationSteps, mobileCardCreationStepTitles } from "@/components/create/mobile/mobileSteps";
...
export default function MobileStepWrapper(props: MobileStepWrapperProps) {
	...
	useEffect(() => {
		setNextButtonIsDisabled(!stepHook.mobileProgress.completedSteps[stepNumber]);
	...
	function NavigationButtonArray() {
		...
		return mobileCardCreationStepTitles.map((title, index) => {
			return ( // There is no nice way to format this due to lint lol
				<Text
					key={index}
					onClick={() => {
					...
					fontFamily={"Barlow"}
					paddingBottom={"15px"}
					color={index - 1 <= mobileProgress.lastContiguousStep() ? "white" : "gray.400"}
				>
		...
		return (
			<Modal isOpen={isOpen} onClose={onClose} isCentered motionPreset="slideInLeft">
				...
				<ModalContent w="100%" h="100%" borderRadius={0} bg="#00000000">
					<ModalHeader>
						<HStack paddingLeft={margin} paddingRight={margin} paddingTop="8%" alignItems="center" align="center">
							<Text fontSize="28" fontStyle="italic" fontFamily="'Barlow Condensed', sans-serif" textColor="green.100">JUMP TO STEP</Text>
							<Spacer />
							<CloseButton onClick={onClose} size="xl" />
						</HStack>
						<Divider marginTop="14px" marginBottom="4%" bgColor="gray.400"/>
					</ModalHeader>
					<ModalBody>
	...
	return (
		<>
			...
			>
				...
				<HStack w={"100%"} justifyContent={"space-between"} marginBottom={"15px"}>
					...
					<Text
						...
						color="white"
						fontFamily={"Barlow"}
					>{mobileCardCreationStepTitles[stepNumber]}</Text>
							...
							onClick={async() => {
								if(!onLastStep) {
									...
									*/
									...
									const result = await submitCardWithAuth(props.entireCardRef, props.cardBackRef, props.currentInfo, userID);
