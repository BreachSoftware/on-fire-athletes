...
export default function MobileUpload({ type }: MobileStep3Props) {
	...
	const [ frontPhotoURL, setFrontPhotoURL ] = useState(card.curCard.frontPhotoURL);
	...
	const [ frontPhotoDimensions, setFrontPhotoDimensions ] = useState({ width: 0, height: 0 });
	...
	async function processPhotoSelect(files: FileList) {
		...
		try {
			...
			reader.onloadend = async() => {
				...
				photoFileName.current = `https://gamechangers-media-uploads.s3.amazonaws.com/image/${ filename }`;

				// Upload the image to S3
				await uploadAssetToS3(filename, blob, "image", "image/png");

				setFrontPhotoDimensions({ width: parseInt(imageWidth!), height: parseInt(imageHeight!) });
	...
	async function processVideoSelect(files: FileList) {
		...
		videoFileName.current = `https://gamechangers-media-uploads.s3.amazonaws.com/video/${ filename }`;
	...
	function SignatureRow() {
		...
		return (
			<HStack w={"100%"} h={"48px"}>
				<SignatureModal buttonText="add Signature" width={"100%"} inlineImage height={"100%"} auth={auth}/>
			</HStack>
		);
	}
			...
			<VStack w={"100%"} justifyContent={"center"}>
				<Box w={"75%"}>
					<UploadRow
						...
						fileName={card.curCard.frontPhotoURL !== "" ? card.curCard.frontPhotoURL : photoFileName.current}
					/>
				</Box>
				<Text mt={1} fontSize={"9px"} fontStyle={"italic"} fontWeight={"bold"}> File Specs: PNG, JPG, MAX 100MB</Text>
				<Text fontSize={"9px"} fontStyle={"italic"} fontWeight={"bold"}>
			...
			<VStack w={"100%"} justifyContent={"center"}>
				<Box w={"75%"}>
					<UploadRow
						...
						fileName={card.curCard.backVideoURL !== "" ? card.curCard.backVideoURL : videoFileName.current}
					/>
				</Box>
				<Text mt={1} fontSize={"9px"} fontStyle={"italic"} fontWeight={"bold"}> File Specs: MP4, MOV, MAX 100MB</Text>
				<Text fontSize={"9px"} fontStyle={"italic"} fontWeight={"bold"}>
	...
	case "sig":
		return (
			<VStack w={"100%"} justifyContent={"center"}>
				...
				<Text mt={1} fontSize={"9px"} fontStyle={"italic"} fontWeight={"bold"}> Turn phone sideways</Text>
			</VStack>
		);
	default:
		break;
