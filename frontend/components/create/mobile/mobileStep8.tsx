	...
	function backgroundSelector(index: number) {
		return (
			<Button
				...
				height={"80px"}
				width={"22vw"}
				border={card.curCard.selectedBackground === index ? "2px solid var(--chakra-colors-green-100)" : ""}
				borderRadius={"5px"}
				p="5px"
				flexShrink={0}
				onClick={() => {
			...
			>
				<Box position="relative" height="100%" width="100%">
					<Image
						...
						borderRadius={"5px"}
						style={{ filter: `brightness(${card.curCard.selectedBackground === index ? "100%" : "70%"})` }}
						draggable={false}
					/>
