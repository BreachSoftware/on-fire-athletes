import { Box, Text, Button, Flex, Heading, useBreakpointValue } from "@chakra-ui/react";
...
export default function InTheNews(props: InTheNewsProps) {
	...
	return (
		<Box pos={"relative"} overflow="hidden" minH={{ base: "0px", md: "800px" }} h="min-content">
			<Flex p={8} flexDir={"column"} h="100%" justifyContent={"center"} alignItems={"center"}>
				...
				{isMobile ? (
								...
								return (
									<div key={index} style={slideStyle}>
										<Box p={2} alignContent={"center"}>
											<Box ml={index === 0 ? 6 : 0} alignItems={"centers"} maxW={"80%"}>
											...
											<Button
												maxW={{ base: "50%", sm: "50%", md: "30%" }}
												_hover={{
													md: {
														maxW: { base: "60%", sm: "60%", md: "40%" },
														width: { base: "60%", sm: "60%", md: "40%" }
													}
												}}
												variant="next"
												fontSize="14px"
					...
					<Flex justifyContent="space-around" flexDir={"row"} flexWrap={"wrap"}>
						{props.data.map((item) => {
							return (
								<Box key={item.id} maxWidth={{ base: "300px", md: "500px" }} p={4} m={8} mx={2}>
									<PhotoFrame src={item.imageUrl} alt="News image" />
									<Text
										fontFamily="'Barlow Condensed', sans-serif"
										fontSize="47px"
