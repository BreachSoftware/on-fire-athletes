 ...
 * @param color
 */
export async function tint(imageString: string, color: string, inHeader:Headers, useAPI:boolean, inverted?:boolean):Promise<string> {
	...
	if(useAPI) {
		...
		const body = JSON.stringify({ imageData: removedPrefixBase64, hexColor: color, inverted: shouldBeInverted });
	...
	const recolored = await image.color([
		{ apply: "mix", params: [ color, 50 ] }
 ...
 * @param color
 */
export async function recolor(imageString: string, color: string, inHeader:Headers, useAPI: boolean): Promise<string> {
	const image = await Jimp.read(imageString);
	// Verify window type (results in  file not found otherwise)
	...
	if (useAPI) {
		...
		const body = JSON.stringify({ imageData: removedPrefixBase64, hexColor: color });
...
*/
export async function maskImageToCard(imagePath: string, maskPath: string):Promise<string> {
	...
	const masked = await image
		.mask(mask, image.bitmap.width - mask.bitmap.width, image.bitmap.height - mask.bitmap.height)
		.getBase64Async(Jimp.MIME_PNG);
 ...
 * @param height
 */
export async function resize(imageString: string, width: number | null, height: number | null):Promise<string> {
	...
	// Resize the image
	const resized = await image.resize(width ? width : Jimp.AUTO, height ? height : Jimp.AUTO).getBase64Async(Jimp.MIME_PNG);
