...
export default function Filter(props: FilterProps) {
 	...
 	*/
	function updateFilterTags(cards: TradingCardInfo[], filterTags: { title: string; value: boolean; present: boolean;}[], filterProperty: string) {
		...
		const selectedSports = currentFilter.curFilter.sport.tags.filter((tag) => {
			return tag.value;
		}).map((tag) => {
			return tag.title;
		});
		...
		for (let i = 0; i < cards.length; i++) {
			...
			if (filterProperty === "createdAt") {
				...
				cardValue = new Date(card.createdAt * 1000).getFullYear().toString();
			}
	...
	useEffect(() => {
		const filterProperties = [ "sport", "position", "careerLevel", "createdAt" ];
		const filterTags = [
		...
		const updatedFilter = {
			...
			sport: { ...currentFilter.curFilter.sport, tags: updatedTagsArrays[0] },
			position: { ...currentFilter.curFilter.position, tags: updatedTagsArrays[1] },
			careerLevel: { ...currentFilter.curFilter.careerLevel, tags: updatedTagsArrays[2] },
			yearCreated: { ...currentFilter.curFilter.yearCreated, tags: updatedTagsArrays[3] },
		};
	...
	const filterMainContent = (
		...
		>
			...
			<Flex width={"100%"} direction={"row"}>
				...
				{Object.keys(currentFilter.curFilter).slice(1).some((key) => {
					const filter = currentFilter.curFilter[key as keyof typeof currentFilter.curFilter];
					if (typeof filter === "object" && filter !== null && "tags" in filter) {
					...
					return false;
							...
							<Text
								...
								cursor={"pointer"}
								onClick={() => {
									const updatedFilter = { ...currentFilter.curFilter };
									for (const key in updatedFilter) {
										if (updatedFilter.hasOwnProperty(key)) {
											const filterItem = updatedFilter[key as keyof typeof updatedFilter];
											if (typeof filterItem !== "string") {
												for (let i = 0; i < filterItem.tags.length; i++) {
			...
			{Object.keys(currentFilter.curFilter).slice(1).map((key, index) => {
				const filter = currentFilter.curFilter[key as keyof typeof currentFilter.curFilter];

				// Check if filter is an object and has 'tags' property
				if (typeof filter === "object" && filter !== null && "tags" in filter) {
					const tags = (filter as { tags: { title: string; value: boolean; present: boolean; }[] }).tags.filter((tag) => {
						return tag.present;
					});
