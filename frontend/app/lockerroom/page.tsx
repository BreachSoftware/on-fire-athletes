...
import { Box, HStack, VStack, Spinner, useToast, Flex, Menu, Button, MenuButton, MenuItem, MenuList, useBreakpointValue } from "@chakra-ui/react";
...
export default function LockerRoom() {
	...
	const titleHeight = useBreakpointValue({ base: 70, sm: 85, md: 100, xl: 150 });
	...
	function filterCards(card: TradingCardInfo): boolean {
		...
		const filterCategories = [ "sport", "careerLevel", "position", "yearCreated" ];
		...
		return filterCategories.every((category) => {
			...
			const tags = filter.curFilter[category as keyof Omit<FilterInfo, "type">].tags;
			...
			const isOtherCard = category === "position" && selectedTags[0].title === "Other";
	...
	useEffect(() => {
		if (filter.curFilter.type === "1") {
			...
			setFilterableCards(cards.filter((card: TradingCardInfo) => {
	...
	function sortCards() {
		...
		case "Alphabetical":
			return [ ...filterableCards ].sort((a: TradingCardInfo, b: TradingCardInfo) => {
					...
					<Box w={showSidebar ? "calc(100vw - 155px)" : "100vw"}>
						...
						<VStack width={"100%"}>
							...
							>
								...
								<Flex>
									<Image src={locker} priority alt="Locker" height={titleHeight} />
									<Image src={room} alt="Room" height={titleHeight} />
								</Flex>
							</HStack>
										...
										>
											...
											>
												...
												<MenuList
													...
													border={0}
												>
													<MenuItem backgroundColor={"#364B47"} onClick={() => {
														return handleSortOption("Recent");
													}}>Recent</MenuItem>
													<MenuItem backgroundColor={"#364B47"} onClick={() => {
														return handleSortOption("Alphabetical");
													}}>Alphabetical</MenuItem>
													<MenuItem backgroundColor={"#364B47"} onClick={() => {
														return handleSortOption("Reverse Alphabetical");
													}}>Reverse Alphabetical</MenuItem>
													<MenuItem backgroundColor={"#364B47"} onClick={() => {
														return handleSortOption("Trending");
													}}>Trending</MenuItem>
												</MenuList>
											</Menu>
										</Flex>
