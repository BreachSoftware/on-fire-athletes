...
function DesktopSubNav({ label, href }: NavItem) {
	return (
		<Box as="a" href={href} role={"group"} display={"block"} rounded={"md"} p={1} transition={"all .3s ease"} _hover={{ bg: NAV_COLORS.darkGreen }}>
			<Stack direction={"row"} align={"center"}>
				<Box>
					<Heading size={"sm"} color={NAV_COLORS.white} fontWeight={500}>
						{label}
					</Heading>
				</Box>
				<Flex
					...
					align={"center"}
					flex={1}>
					<Icon color={NAV_COLORS.green} w={5} h={5} as={ChevronRightIcon} />
				</Flex>
			</Stack>
		</Box>
	);
		...
		<Flex height={"min-content"} width={"100%"}>
			<Stack direction={"row"} width="100%">
				{NAV_ITEMS.slice(0, 3).map((navItem) => {
					const displayValue = displayTabs(navItem.label, auth.isAuthenticated);
					...
					return (
						<Box
							...
							<Popover placement={"bottom"} trigger={"hover"}>
								<PopoverTrigger>
									<Link
										href={navItem.label === "Sign Out" ? undefined : props.hreffunc ? props.hreffunc(navItem.href || "") : undefined}
										...
										<Flex
											...
											{navItem.children && <Icon as={ChevronDownIcon} w={5} h={5} ml={-3} mr={3} transition={"all .3s ease"} />}
										</Flex>
									</Link>
								</PopoverTrigger>

								{navItem.children && (
									<PopoverContent border={0} borderRadius={0} p={4} zIndex={4} bg={NAV_COLORS.green} textTransform="uppercase" width={"fit-content"}>
										<Stack>
			...
			<Flex justifyContent="flex-end" width="100%" direction="row">
				{NAV_ITEMS.slice(3).map((navItem) => {
					return (
						<Box
							key={navItem.label}
							display={displayTabs(navItem.label, auth.isAuthenticated)}
							onClick={
								navItem.label === "Sign Out"
									? async () => {
											await auth.signOut();
							...
							<Link
								...
								<Flex
									...
									{navItem.children && <Icon as={ChevronDownIcon} w={5} h={5} transition={"all .3s ease"} />}
								</Flex>
							</Link>
						</Box>
					);
	...
	return (
		<Box zIndex={99} mb={isMobile ? 4 : 0}>
			<Flex
				...
				width={"100%"}
				borderBottom={isMobile && windowLocation === "/create/card_creation" ? "2px solid #161E1C" : "none"}>
				<Flex flex={1} align={"center"} justify={"space-between"} zIndex={1} position={"relative"}>
					{/* Logo */}
					<Link href="/">
						<Image src={OnFireLogo.src} alt="Logo" width={"59px"} mr={{ base: "0px", md: "16px" }} ml={{ base: "0px", md: "15px" }} />
					</Link>

					{/* Navbar Items */}
					<Flex display={{ base: "none", md: "flex" }} ml={5} width={"100%"}>
						<DesktopNav
					...
					{isMobile && <SideBarHamburger onClick={onToggle} showXIcon={isOpen} onSubClose={onToggle} />}
				</Flex>
			</Flex>

			{/* Spacer only for mobile */}
			{/* Removed for now */}
			{/* { props.noDivider ? null : <Spacer display={{ base: "flex", md: "none" }} h="1px" bg="#707070" /> } */}
			<SideDrawer isOpen={isOpen} onClose={onToggle} placement={"right"} size={"xl"} isMobile={isMobile} />
		</Box>
	);
}
