...
export default function SendMyCardModal(props: SendMyCardModalProps) {
	...
	async function tradeCardEmail() {
		...
		const fetcheduuid = fetch(apiEndpoints.getUUID(), uuidrequestOptions) // change to prod
		...
		if (newOwnUUID === accountUUID) {
			...
			await fetch(apiEndpoints.assignCardEmail(), requestOptions)
				...
				.then(() => {
					toast({
						title: "Card Sent!",
						description: "Once the recipient accepts the card from their email, it will be added to their collection.",
						status: "success",
						duration: 5000,
						isClosable: true,
					});
	...
	const emailInputComponent = (
		<Flex direction={"column"} gap="25px">
			<Flex direction={"column"} mb="1em">
				<Text fontSize="16px" mb="10px">Recipient&apos;s Email Address:</Text>
				<Input
					type="email"
			...
			<Flex direction="row" w="100%" gap="25px">
				...
				<Button variant="next" isLoading={isLoading} onClick={tradeCardEmail}>
					<Text>Send</Text>
				</Button>
			</Flex>
		</Flex>
	...
	return (
		...
		<CardActionModal
			...
			title={"Light it up!"}
			subtitle={"Enter the recipient's email address to send them a copy of this card."}
