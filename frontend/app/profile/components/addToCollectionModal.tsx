...
export function AddToCollectionModal(props: AddToCollectionModalProps) {
	...
	async function requestCardEmail() {
		...
		const generatedByProfile = await fetch(
			`${apiEndpoints.getUser()}?uuid=${props.currentCard.generatedBy}`, uuidrequestOptions
		);
		...
		const requesterProfile = await fetch(
			`${apiEndpoints.getUser}?uuid=${props.currentUserId}`, uuidrequestOptions
		);
		...
		const addToCollectionResponse = await fetch(apiEndpoints.addToCollectionEmail(), requestOptions);
	...
	return (
		<CardActionModal
			...
			title={"Light it up!"}
			subtitle={`Request to add "${props.currentCard.firstName} ${props.currentCard.lastName}" to your collection?`}
