...
import { Divider, Collapse, Flex, Grid, Spacer, Text, useDisclosure, useToast, Box, Spinner } from "@chakra-ui/react";
...
import OnFireCard, { OnFireCardRef } from "@/components/create/OnFireCard/OnFireCard";
...
export default function ProfileAlbumTab(props: ProfileAlbumProps) {
	...
	const [ currentSerializedCard, setCurrentSerializedCard ] = useState(new SerializedTradingCard(0, new TradingCardInfo({})));
	...
	const currentCardYearCreated = new Date(currentCard.createdAt * 1000).getFullYear();
	const onFireCardRef = useRef<OnFireCardRef | null>(null);
	const isBuyableCard = currentCard.price !== 0 && currentCard.currentlyAvailable > 0 && !props.privateView;
	...
	const isMobile = (/mobile|android|ipad|iphone|ipod|blackberry|iemobile|opera mini/i).test(userAgent);
	...
	async function tradeCardShare() {
		...
		if (!senderUUID || !generatedByUUID || !cardUUID) {
			console.error("Error sharing: No user UUID, no generatedBy UUID, or card UUID found");
			toast({
				title: "Error Sharing!",
				description: "An error occurred while sharing the card. Please try again later.",
				duration: 5000,
				isClosable: true,
				status: "error",
			});
		...
		if (buttonShouldBeDisabled) {
			return (
				<ViewedCardActionButton
					...
					onClick={() => {}}
				>
					<Text fontFamily={"Barlow"} fontSize="16px" fontWeight="600" letterSpacing="2px">
						CARD UNAVAILABLE
					</Text>
				</ViewedCardActionButton>
			);
			...
			if (currentTab === TabName.Created || currentTab === TabName.Traded) {
				return (
					<ViewedCardActionButton
						opacity={"1"}
						hover={{ bgColor: "#CCC",
						...
						onClick={isMobile ? tradeCardShare : processSendCardModalOpen}
					>
						<Text fontFamily={"Barlow"} fontSize="18px" fontWeight="600" letterSpacing="2px">
							SEND MY CARD
						</Text>
					</ViewedCardActionButton>
				);
				...
				return (
					...
					>
						...
						<Text
							...
							color={"white.100"}
						>
							By purchasing a digital card, you will also receive a physical card in the mail.
						</Text>
					</Flex>
				);
			}
	...
	return (
		<>
			<Flex w={{ base: "90%", md: "80%", lg: "70%" }} direction="column" align="center" mt={{ base: "0px", md: "15px" }}>
				<Collapse in={viewCardClicked} animateOpacity>
					<Flex w="75vw" direction="column" align="center" mt="-50px">
						<Flex
							w="max-content"
							borderRadius="10px"
						...
						>
							...
							>
								<Box
									transform={{ base: "scale(0.6)", sm: "scale(0.8)", md: "scale(1)" }}
									margin={{ base: "-35px", sm: "60px" }}
									marginBottom={{ base: "-95px", sm: "10px" }}
								>
									<Element name="cardElement" className="element" />
									<OnFireCard
							...
							<Flex w="100px" h="100%" />
							<Flex direction="column" w="75vw" maxW="500px" justify="center" mb={{ base: "10px", md: "60px" }} userSelect={"none"}>
								<Text
								...
								<Text
									...
									mb="8px"
								>
									{currentCard.sport}, {currentCard.position} / {currentCard.teamName}
								</Text>
								<Text lineHeight="30px" letterSpacing="0.25px" mb="30px">
								...
								>
									{currentCard.price !== 0 ? (
										<>
											<Text fontFamily="Barlow" fontWeight={500}>Price</Text>
											<Text color={"gray.400"} fontFamily="Barlow" fontWeight={400}>
												${currentCard.price.toFixed(2)}
											</Text>
										</>
									) : null}
									{ currentTab !== TabName.Created && currentSerializedCard.serialNumber ? (
										<>
											<Text fontFamily="Barlow" fontWeight={500}>Card Number</Text>
											<Text color={"gray.400"} fontFamily="Barlow" fontWeight={400}>
												{currentSerializedCard.serialNumber} of {currentCard.totalCreated}
											</Text>
										</>
									) : (<>
										<Text fontFamily="Barlow" fontWeight={500}>Cards Available</Text>
										<Text color={"gray.400"} fontFamily="Barlow" fontWeight={400}>
											{currentCard.currentlyAvailable} of {currentCard.totalCreated}
										</Text>
									</>)
									}
									<Text fontFamily="Barlow" fontWeight={500}>Year Created / Package Type</Text>
									<Text color={"gray.400"} fontFamily="Barlow" fontWeight={400}>
										{currentCardYearCreated} / {currentCard.price == 0 ? "Rookie Package" : "All-Star Package"}
									</Text>
								</Grid>
								{/* Button code */}
								{actionButtonUI()}
								...
								{!isMobile ? (
									<SendMyCardModal
										...
										onOpen={processSendCardModalOpen}
								...
								<AddToCollectionModal
									...
									onClose={addToCollectionModal.onClose}
									currentCard={currentCard}
									fromName={props.currentUserName}
						...
						<Divider w="100%" maxW={"1250px"} h="2px" borderRadius="full" bgColor="gray.100" opacity="25%" my="20px" />
					</Flex>
				</Collapse>
				<Flex
					w={{ base: "90%", md: "75vw" }}
				...
				>
					<Text
						...
						textColor={currentTab === TabName.Created ? "green.100" : "white"}
						textDecor={currentTab === TabName.Created ? "underline" : "none"}
						onClick={() => {
							setCurrentTab(TabName.Created);
							setViewCardClicked(false);
							setCurrentCard(new TradingCardInfo({}));
							setCurrentSerializedCard(new SerializedTradingCard(0, new TradingCardInfo({})));
						}}
					>
						Created
					</Text>
					<Text
						...
						fontSize="20px"
						_hover={{ cursor: "pointer" }}
						textColor={currentTab === TabName.Traded ? "green.100" : "white"}
						textDecor={currentTab === TabName.Traded ? "underline" : "none"}
						onClick={() => {
							setCurrentTab(TabName.Traded);
							setViewCardClicked(false);
							setCurrentCard(new TradingCardInfo({}));
					...
					<Text
						...
						fontSize="20px"
						_hover={{ cursor: "pointer" }}
						textColor={currentTab === TabName.Bought ? "green.100" : "white"}
						textDecor={currentTab === TabName.Bought ? "underline" : "none"}
						onClick={() => {
							setCurrentTab(TabName.Bought);
							setViewCardClicked(false);
							setCurrentCard(new TradingCardInfo({}));
				...
				{currentTab === TabName.Created && (
					isLoading ?
						...
						<CardListGrid
							...
							onSendCardModalOpen={isMobile ? tradeCardShare : processSendCardModalOpen}
							loginModalOpen = {loginModal.onOpen}
							onAddToCollectionModalOpen={addToCollectionModal.onOpen}
