...
import { Flex, Grid, GridItem, Text, SkeletonText, Input, useToast, ToastId, UseToastOptions } from "@chakra-ui/react";
		...
		<>
			<Flex w="70%" mt="50px" direction="column">
				...
				<Grid
					...
					alignItems={"center"}
				>
					{ editable && isLoaded && <GridItem h="full" minH="232px">
						<ProfileNewMedia isLoading={fileIsLoading} handleClick={handleButtonClick} />
					</GridItem>
					}
					{media.map((media, index) => {
						return (
							<GridItem key={index}>
								<ProfileBioMedia media={media} isEditable={editable} handleDelete={deleteFromMediaArray} />
							</GridItem>
						);
					})}
					<Input
