...
export default function SignUp() {
	...
	useEffect(() => {
		...
		if (card) {
			...
			if (currentTime - cardTime > 86400000) {
				...
				checkForTrade(generatedByUUID, cardSentUUID, senderUUID, null, null, auth, toast);
			}
		} else {
			checkForTrade(generatedByUUID, cardSentUUID, senderUUID, null, null, auth, toast);
		}
	 ...
	 */
	async function confirmEmail(code: string, email: string, password: string, firstName: string, lastName: string) : Promise<boolean> {
		if (code.trim().length !== 0) {
			...
			if (confirmed) {
				...
				if(res.success) {
					...
					if (res.message === "Successful sign in but unsuccessful profile check") {
					...
					if (card) {
						...
						await checkForTrade(generatedByUUID, cardSentUUID, senderUUID, null, null, auth, toast);
					}
					return true;
				}
	...
	function checkLoginQueryPrams() {
		if(generatedByUUID && cardSentUUID && senderUUID) {
			router.push(`/login?generatedByUUID=${generatedByUUID}&cardUUID=${cardSentUUID}&fromUUID=${senderUUID}`);
		} else {
			router.push("/login");
		}
	}
	...
	return (
		<>
			...
			>
				...
				<Flex w="100%" flexDirection={"column"} overflowY="scroll">
					<NavBar />
					<LoginHeader title="Sign Up" subtitle="Create an account to get started!" />
					<SignUpUI
						onClick={() => {
