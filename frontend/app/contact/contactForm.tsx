...
export default function ContactForm() {
    ...
    async function processAttachmentSelect(files: FileList) {
        ...
        await uploadAssetToS3(
            filename,
            await b64toBlob(myPhoto),
            "attachments",
            filetype,
        );
        setAttachment(
            `https://onfireathletes-media-uploads.s3.amazonaws.com/attachments/${filename}`,
        );
    ...
    async function sendEmail() {
        ...
        const contactusRequest = await fetch(
            apiEndpoints.contactEmail(),
            requestOptions,
        );
        if (contactusRequest.status === 200) {
            ...
            toast({
                title: "Request Sent",
                description:
                    "The OnFire team has been notified of your message.",
                status: "success",
                duration: 5000,
                isClosable: true,
            });
