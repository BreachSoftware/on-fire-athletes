...
export function BackToCheckoutModal() {
	...
	const [ cardExists, setCardExists ] = useState(TradingCardInfo.cardExistsInLocalStorage());
	const [ currentCard, setCurrentCard ] = useState<TradingCardInfo>(TradingCardInfo.loadCard());
	...
	useEffect(() => {
		 ...
		 */
		async function retrieveCard(cardUUID: string, generatedBy: string): Promise<TradingCardInfo> {
		...
		getLoggedInUserID().then((id) => {
			if (id) {
				...
				if (cardExists && storedCardInfoJSON) {
					...
					const cardInfoInCookie: TradingCardInfo = JSON.parse(JSON.parse(storedCardInfoJSON).value);
					...
					retrieveCard(cardInfoInCookie.uuid, cardInfoInCookie.generatedBy).then((card) => {
						setCurrentCard(card);
						setCardLoaded(true);
					});
				}
	...
	function onReturnToCheckout() {
		router.push(`/checkout${cardOwnedByUser ? "" : "?buyingOtherCard=true"}`);
	}
		...
		<>
			{cardExists && cardLoaded && (
				<CardActionModal
					...
					subtitle={"Would you like to return to the checkout screen?" }
