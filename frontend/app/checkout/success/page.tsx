"use client";
import { Flex, HStack, Box, VStack, Spinner, Text } from "@chakra-ui/react";
...
export default function SuccessfulCardCreationPage() {
	const [ cardSuccessfullyCreated, setCardSuccessfullyCreated ] = useState(false);
	...
	*/
		...
		if (typeof window !== "undefined") {
			...
			if (paymentIntentID) {
				...
				if (status === "succeeded") {
					...
					if (!boughtOtherCard) {
						await updatePaymentStatus(generatedBy, uuid, PaymentStatus.SUCCESS);
					}
					// Clear the card
	...
	return (
		<Box w={"100vw"} minH={{ base: "110vh", sm: "100vh" }} bgGradient={"linear(180deg, gray.1200 0%, gray.1300 100%) 0% 0% no-repeat padding-box;"}>
			<HStack w="100%" h={{ base: "100%", lg: "100vh" }} align="top">
				<VStack w="100%" h="100%" align="center">
...

					{
						cardSuccessfullyCreated ?
							...
							>
								<Flex h="100%" alignItems="center" fontSize="3xl" fontWeight="bold" color="white">
									<OrderCompleteContent cardUUID={uuid} generatedByUUID={generatedBy} buyingOtherCard={buyingOtherCard} />
								</Flex>
							</Flex> :
