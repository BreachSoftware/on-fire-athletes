"use client";
import { Box, Flex, Grid, GridItem, HStack, Spinner, useBreakpointValue, VStack } from "@chakra-ui/react";
...
import { Environment, environmentManager } from "@backend/EnvironmentManager/EnvironmentManager";
...
export default function CheckoutPage() {
	...
	useEffect(() => {
		if (checkoutStep === 2 && !buyingOtherCard) {
			...
			// Set the main package item
				...
				if (co.checkout.packageName === "allStar" || co.checkout.packageName === "mvp") {
					includedPhysicalAddOn = {
						title: "Included Physical Card",
						card: onFireCard,
						numberOfCards: 1,
						numberOfOrders: co.checkout.packageName === "allStar" ? 1 : 5,
						price: 0.00,
					};

					// If there are additional physical cards beyond the free ones, set the physical card add-on
					if (co.checkout.packageName === "mvp" && co.checkout.physicalCardCount >= 6) {
						physicalAddOn = {
				...
				co.setCheckout({
					...checkout,
					cart: [ mainPackage, includedPhysicalAddOn, digitalAddOn, physicalAddOn ].filter((item) => {
						return item !== null;
					}),
				});
			}
	...
	return (
		<RainbowKitProvider theme={darkTheme()}>
			<Flex w={"100vw"} minH={{ sm: "100vh" }} bgGradient={"linear(180deg, gray.1200 0%, gray.1300 100%) 0% 0% no-repeat padding-box;"}>
				{/* The stepNum code is to account for running out of space in the large screen orientation */}
				<HStack w="100%" h={{ base: "100%", lg: checkout.stepNum === 0 ? "145vh" : "100vh", xl: "100vh" }} align="top">
					<VStack w="100%" h="100%" align="center" overflowX="hidden" overflowY="scroll">
						...
						{ showSpinner ?
							...
							checkout.stepNum === 0 ?
								...
								checkout.stepNum === 1 ?
									...
									(screenTooSmall ?
										(
											<VStack w="100%" h="100%" gap="25px" align="top" paddingX={{ base: "25px", lg: "50px" }}>
												<CheckoutHeader />
												<CheckoutItemsInCart items={itemsInCart} buyingOtherCard={buyingOtherCard} />
												<Elements stripe={stripePromise}>
													<CheckoutStepWrapper onFireCard={onFireCard} buyingOtherCard={buyingOtherCard} />
												</Elements>
											</VStack>
										) :
										(
