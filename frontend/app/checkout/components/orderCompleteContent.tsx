...
import { VStack, Button, Link, Text, Flex, useToast, useBreakpointValue } from "@chakra-ui/react";
...
export default function OrderCompleteContent(props: OrderCompleteContentProps) {
	...
	const [ createNewCardButtonLoading, setCreateNewCardButtonLoading ] = useState(false);
	const [ viewLockerRoomButtonLoading, setViewLockerRoomButtonLoading ] = useState(false);
	...
	function shareCardOnSocialMedia() {
		if (typeof window !== "undefined" && navigator.share) {
			navigator.share({
				...
				url: `https://gamechangers.zenithsoftware.dev/profile?user=${props.generatedByUUID}&card=${props.cardUUID}`,
			}).then(() => {
				toast({
			...
			navigator.clipboard.writeText(`https://gamechangers.zenithsoftware.dev/profile?user=${props.generatedByUUID}&card=${props.cardUUID}`).then(() => {
				toast({
				...
				console.error("Error copying card link to clipboard: ", error);
				toast({
			...
			<VStack gap="20px">
				<VStack alignItems={"center"}>
					<Text
						...
						fontWeight={"light"}
						fontStyle={"italic"}
						fontSize={{ base: "20px", md: "25px", lg: "32px", xl: "44px" }}
					...
					<Text
						...
						fontWeight={"medium"}
						fontSize={{ base: "45px", md: "50px", lg: "60px", xl: "79px" }}
						textAlign="center"
						lineHeight={"1.2"}
						pb={isMobile ? "15px" : "0"}
					>
					...
					<Button
						...
						onClick={() => {
							...
							if (props.buyingOtherCard) {
								router.push(`/profile?user=${userId}&card=${props.cardUUID}&tab=bought`);
							} else {
					...
					>
						<Flex alignItems={"center"}>
							<Text letterSpacing={"2px"} textTransform={"uppercase"}>
								{props.buyingOtherCard ? "VIEW BOUGHT CARD" : "START COLLECTING"}
							</Text>
