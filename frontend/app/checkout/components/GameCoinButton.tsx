...
import { Box, Button, Flex, HStack, Popover, PopoverBody, PopoverContent, PopoverTrigger, useBreakpointValue, Text } from "@chakra-ui/react";
...
export default function GameCoinButton() {
	...
	return (
		<HStack>
			...
			{screenTooSmall && <Box visibility={"hidden"}><FontAwesomeIcon icon={faInfoCircle} color={"#27CE00"} /></Box>}
			{/* Game Coin Button */}
			<Flex alignItems="end">
				{ transferContext.address ? (
					<Button
						id="submit"
						type="submit"
						disabled={transferContext.isFetching || transferContext.isPending}
						variant="back"
						// If the transfer is fetching or pending and didn't finish (such as an error occurring), we want to show the loading spinner
						isLoading={(transferContext.isFetching || transferContext.isPending) && !transferContext.isComplete}
						textColor={"white"}
						onClick={() => {
					...
					>
						...
						<Flex alignItems={"center"}>
							<ChevronRightIcon boxSize={"30px"} mr={"-10px"} color={"#27CE00"} />
						</Flex>
					</Button>
				) : (
						...
						{({ openConnectModal }) => {
							return (
								<Button
									...
									disabled={transferContext.isFetching || transferContext.isPending}
									variant="back"
									isLoading={transferContext.isFetching || transferContext.isPending}
									textColor={"white"}
									onMouseEnter={() => {
										if (screenTooSmall) {
			...
			<Popover placement={screenTooSmall ? "auto" : "right"}>
				<PopoverTrigger>
					...
					>
						<FontAwesomeIcon icon={faInfoCircle} color={"#27CE00"} />
					</Box>
				</PopoverTrigger>
				<PopoverContent bgColor="green.600">
					<PopoverBody>
						<Text color="white" fontWeight={"medium"} fontSize={"20px"} fontStyle={"italic"} fontFamily={"Barlow Condensed"}>
