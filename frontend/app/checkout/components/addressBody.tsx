import { Flex, Input, Stack, VStack, Text, Select, Checkbox, HStack } from "@chakra-ui/react";
...
export default function AddressBody({ addressType } : AddressBodyProps) {
	...
	const shippingOrBilling = addressType === AddressType.SHIPPING ? "shippingAddress" : "billingAddress";
	...
	return (
		<Flex flexDirection="column" gap="20px">
			...
			<Flex gap="10px" flexDirection={"column"}>
				...
				<Stack direction={{ base: "column", md: "row" }} gap={{ base: 4, md: 12 }}>
					<Input
						...
						value={checkout[shippingOrBilling].firstName}
						variant={"checkout"}
						onChange={(e) => {
							addToAddressInfo("firstName", e.target.value);
						}} />
					<Input
						placeholder="Last Name*"
						value={checkout[shippingOrBilling].lastName}
						variant={"checkout"}
						onChange={(e) => {
							addToAddressInfo("lastName", e.target.value);
						}} />
				</Stack>
			</Flex>
			...
			<Flex gap="10px" flexDirection={"column"}>
				<Stack direction={{ base: "column", md: "row" }} gap={{ base: "20px", md: 8 }}>
					{/* City input field */}
					...
					<HStack flexGrow={1} gap={{ base: "20px", md: 8 }} w={"100%"}>
						{/* State dropdown */}
						<VStack w={{ base: "100%", md: "50%" }} align={"start"}>
							...
							<Select
								...
								textColor={checkout[shippingOrBilling].state === "" ? "gray.400" : "white"}
						...
						<VStack w={{ base: "100%", md: "70%" }} align={"start"}>
							...
							<Input
								...
								onChange={(e) => {
									...
									addToAddressInfo("zipCode", formattedZipCode);
								}}
							/>
						</VStack>
					</HStack>
