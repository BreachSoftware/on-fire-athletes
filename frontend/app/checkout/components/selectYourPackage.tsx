...
export default function SelectYourPackage() {
	...
	const AllStarInfo = {
		...
		price: 39.99,
		details: [
			{ text: "Receive 1 Physical Card With 3D and A/R Interactivity", indent: false },
			{ text: "Receive 15 Digital Cards", indent: false },
			{ text: "Ability to SELL or Trade Your Cards", indent: false },
			{ text: "Athlete Receives 80% of Sales", indent: true },
			{ text: "Ability to Collect Cards", indent: false },
			{ text: "Build Your Own Athlete Profile", indent: false }
		],
	};
	...
	const MVPInfo = {
		...
		price: 79.99,
		details: [
			{ text: "Receive 5 Physical Cards With 3D and A/R Interactivity", indent: false },
			{ text: "Receive 25 Digital Cards", indent: false },
			{ text: "Ability to SELL or Trade Your Cards", indent: false },
			{ text: "Athlete Receives 80% of Net Sales*", indent: true },
			{ text: "Ability to Collect Cards", indent: false },
			{ text: "Build Your Own Athlete Profile", indent: false }
		],
	};
				...
				{[ RookieInfo, AllStarInfo, MVPInfo ].map((pkg, index) => {
					return (
						<Flex
							...
							flexDirection={"column"}
							borderWidth={"2px"}
							borderColor={checkout.packageName === pkg.databaseName ? "green.100" : "transparent"}
							boxShadow={checkout.packageName === pkg.databaseName ? glowEffect : "none"}
							...
							onClick={() => {
								...
								curCheckout.setCheckout({
									...checkout,
									packageName: pkg.databaseName as "rookie" | "allStar" | "mvp",
								});
							}}
							cursor={"pointer"}
							...
							_hover={{
								md: {
									backgroundColor: checkout.packageName !== pkg.databaseName ? "#1C2421" : "#171C1B",
									borderColor: checkout.packageName !== pkg.databaseName ? "green.300" : "green.100",
								}
							}}
						>
							{/* Header and Price */}
							...
							>
								<Text
									...
									color="white"
									fontSize={{ base: "20px", sm: "25px", md: "30px", xl: "40px" }}
									textAlign={"center"}
									transform={"skew(-5deg)"}
								>
									{pkg.title}
							...
							<Divider borderWidth={2} borderColor="#000000" marginY="10px" />
							...
							>
								<Text fontFamily={"Barlow Condensed"} fontSize={"18"} textColor={"#F8F8F8"} marginBottom={"5px"}>
								INCLUDES:
								</Text>
								...
								{pkg.details.map((detail, index) => {
									return (
										<Flex
											...
											alignItems={"center"}
											gap={"10px"}
											style={{ paddingLeft: detail.indent ? "25px" : "0" }}
										>
							...
							>
								{pkg.title === "MVP" && (
									<>
										<Text fontFamily={"Barlow Condensed"} fontSize={"16"} textColor={"#F8F8F8"}>
										ADD-ONS:
										</Text>
										...
										>
											<AddOn
												title={"Physical A/R Card"}
												price={checkout.physicalCardPrice.toFixed(2)}
												value={checkout.packageName === "mvp" ? checkout.physicalCardCount : 0}
												onChange={(value) => {
													curCheckout.setCheckout({
														...checkout,
											...
											/>
											<AddOn
												title={"Additional Digital Card"}
												price={checkout.digitalCardPrice.toFixed(2)}
												value={checkout.packageName === "mvp" ? checkout.digitalCardCount : 0}
												onChange={(value) => {
													curCheckout.setCheckout({
														...checkout,
							...
							<Flex
								...
								paddingLeft="15px"
							>
								<Text fontFamily={"Barlow Condensed"} fontSize={"24"} textColor={"#F8F8F8"}>
									Subtotal: ${
										checkout.packageName === pkg.databaseName ?
											(pkg.price +
											(checkout.digitalCardCount * checkout.digitalCardPrice) +
											(checkout.physicalCardCount * checkout.physicalCardPrice)).toFixed(2) : pkg.price}
								</Text>
									...
									onClick={() => {
										if (checkout.packageName === "rookie") {
											...
											curCheckout.setCheckout({
												...
												physicalCardCount: checkout.physicalCardCount + 5,
												stepNum: checkout.stepNum + 2,
											});
										}
									}}
