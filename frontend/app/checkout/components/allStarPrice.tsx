...
import { Box, Divider, Heading, Input, Text, Image, Flex, Button, InputGroup, useBreakpointValue, InputLeftElement } from "@chakra-ui/react";
...
export default function AllStarPrice() {
	...
	function checkIfPrice(event: React.KeyboardEvent<HTMLInputElement>) {
		...
		if (parseFloat((value + key)) > 10000.00) {
			event.preventDefault();
		}
	}
	...
	return (
		<>
			<Box
				bgColor={{ base: "black", sm: "transparent" }}
				w="100%"
				h="100%"
				...
				>
					...
					>
						...
						<Flex flexDirection={"column"} gap="25px">
							<Flex alignItems={{ base: "center", sm: "start" }} flexDirection={"column"} gap={{ base: "10px", md: "0px" }}>
								<Heading
									fontFamily="Brotherhood"
									color="white"
									fontWeight={"100"}
									fontSize={{ base: "48px", lg: "76px" }}
									letterSpacing={{ base: "2.4px", lg: "3.8px" }}
								>
									Nice Choice!
								</Heading>
								<Heading
							...
							>
								<Heading letterSpacing={"1.3px"} transform={"skew(-7deg)"} fontSize={"26px"} fontFamily={"Barlow Condensed"} fontWeight={600}>
									Set Your Price
								</Heading>
								<InputGroup size="lg">
									<InputLeftElement
										...
										fontFamily={"Barlow Condensed"}
										fontWeight={400}
										paddingTop={{ base: "16px", md:"12px" }}
										paddingLeft={{ base: "13px", md:"10px" }}
										fontSize={"27px"}
                                ...
                                $
									...
									<Input
										...
										_focusVisible={{
											opacity: curCheckout.checkout.cardPrice === "" ? ".65" : "1",
										}}
										fontSize={"28px"}
										textAlign={"left"}
										fontStyle={"italic"}
										...
										backgroundColor={isMobile ? "#121212" : "#303c3a"}
								...
								<Text
									...
									mb="15px"
								>
										*Digital cards must be priced at a minimum of $20.00.
								</Text>
							</Flex>
						</Flex>
					</Flex>
					<Flex w={"100%"}
						...
						gap="25px"
					>
						<Divider alignSelf={"center"} marginTop={{ base: "25px" }} zIndex={0} borderWidth={2} borderColor="gray.1700"/>
						<Flex justifyContent={"center"} gap="20px" marginBottom={"80px"} mr={{ md: "40px" }}>
							{/* Back button to go to the previous step */}
							<Button
								variant={"back"}
								width="100px"
								onClick={() => {
									curCheckout.setCheckout({ ...curCheckout.checkout, stepNum: stepNumber - 1 });
								}}
							>
                        Back
							</Button>
							...
							<Button
								...
								isDisabled={parseFloat(checkout.cardPrice.toString() || "0") < 20}
								onClick={() => {
									// Increment the step number to go to the next step, up to the last step
									if (stepNumber >= 0 && stepNumber < checkoutSteps.length - 1) {
										curCheckout.setCheckout({ ...checkout, cardPrice: parseFloat(checkout.cardPrice).toFixed(2), stepNum: stepNumber + 1 });
									}
								}}
							>
